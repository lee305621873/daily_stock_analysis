...
> 公式校验现支持常见通达信/同花顺风格写法兼容：如 `VAR1:=...; VAR2:=...; XG: VAR1 AND VAR2;` 会自动转换为内部 DSL 表达式；同时兼容部分字段名换行拆分（如 `C\nLOSE`）场景。并新增运行时函数兼容：`DYNAINFO(3/4/5/6/7/8/10/11/12/35/39/40/41)`、`FINANCE(1/2/6/7/30/33/34/35/37/38/40/41/42/46/47)`、`NAMELIKE()`；`DRAW*`/`COLOR*` 绘图语句在选股时会自动忽略（不参与命中判定）。
