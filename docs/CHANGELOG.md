...
- 📊 **多市场技术指标选股器增强**
  - Web 主站新增 `选股` Tab：默认进入公式选股；A 股/港股/美股支持条件选股与公式选股，并支持按市场切换预置板块范围或自定义股票池。
  - Web 交互改为通过扫描范围选择器控制全市场、预置板块和自定义股票池，减少必须额外勾选后才生效的阻塞感；原板块过滤入口从主界面移除，避免与扫描范围重复。
  - A 股全市场扫描新增后台任务 + SSE 进度推送，页面会持续显示扫描进度并在完成后自动刷新结果列表。
  - 公式模式新增页内校验反馈，点击“开始选股”会先自动校验公式，校验失败时直接在页面提示，不再静默无反馈。
  - 选股模式新增 `组合模式（hybrid）`：同一请求可同时使用“公式 + 条件”并按交集命中，前后端模式枚举与校验规则已同步。
  - 选股交互简化：条件编辑器在单输出指标时隐藏“输出”选择；`cross_up / cross_down` 自动切换为“另一指标”对比，降低误配概率。
  - 选股页操作栏改为非固定吸顶，随页面自然滚动，减少滚动时覆盖内容与视觉干扰。
  - 后端新增统一公式 DSL、函数元数据接口与公式校验接口，并沉淀公式语法/函数/准确性文档，便于后续扩展更多技术指标而不重复开发。
  - 后端新增主 API 选股接口与 `market` 请求字段，并对 HK/US 无显式代码池的请求返回明确错误，避免误判为全市场扫描。
  - 选股页新增后端下发的 A/H/US 扫描范围、扫描上限与指标问号提示；A 股板块优先通过 AkShare 拉取真实成分股，失败时回退到维护清单，港股和美股使用后端可配置范围清单。
  - 新增 A 股真实行业/概念板块目录接口与板块成分股预览；前端切到动态板块范围后可搜索具体板块，并按完整成分股执行扫描。
  - 港股与美股补充后端维护的动态行业板块池；前端可按行业选择更贴近真实行业的代表股票池，且后续可无缝替换成新的数据源。
  - 港股与美股行业板块池进一步按“龙头 / 中军 / 弹性”分层维护，并在前端透出分层说明，方便后续持续扩充行业覆盖而不改页面逻辑。
  - 港股与美股动态行业池继续扩充到地产基建、航运物流、博彩旅游、通信设备、公用事业、REITs、航空航天军工、材料化工、汽车出行等板块；前端选中行业后可直接查看每一层的代表代码明细。
...
- 🛠️ **选股页元数据 / 公式校验解耦数据源初始化**
  ...
  - A 股板块成分股链路新增搜狐网页兜底（`AkShare -> Tushare -> Sohu -> 本地维护清单`），并在实时结果数量偏少时自动尝试补全；港股全市场列表新增 `stock_hk_spot_em -> stock_hk_spot` 双路径兜底，减少动态行业板块“最多只有十几只”的异常场景。
  - A 股技术指标选股板块成分展示顺序调整为“优先采用 Sohu 实时顺序”，AkShare/Tushare 仅作为补充来源，不再覆盖主展示顺序。
  - A 股板块成分股进一步改为多源并集聚合（Sohu/AkShare/Tushare/本地维护清单）；`board_catalog` 的 `estimated_count` 优先使用缓存成分股条数，且日志会输出各来源命中数量，便于定位“半导体数量明显偏少”问题。
  - 扫描范围中的 A 股预置板块预览改为优先读取本地缓存真实成分股数量（如半导体 418），不再长期停留在 20 条 fallback 预览。
  - A 股扫描范围新增“缓存驱动自动扩展”能力：会从缓存板块数据自动补充行业/概念范围（默认扩展到 15+）；`refresh_screener_board_cache.py` 新增 `--cn-seed-per-type`，在非 `--cn-all-dynamic` 模式下默认每类预热 12 个板块。
  - 板块预览接口 `GET /api/v1/stocks/screener/boards/preview` 支持全量返回：`limit<=0` 时返回全部成分股；前端默认改为全量预览，不再固定 `20` 条。
  - 修复“已选板块却退回全 A 股扫描”的兼容链路：预置板块范围现在会随扫描请求显式携带 `board_name/board_type`，后端对 `board_filters` 也会优先构建板块并集股票池，并新增 `universe prepared` 日志输出实际扫描来源与数量，便于快速定位是否误扫全市场。
  - 选股输入容错增强：A 股自定义代码池遇到 `002218.300528` / `002218,300528` / `002218300528` 这类粘连输入会自动拆分为合法 6 位代码，不再直接报 `invalid cn codes`；异步任务受理后前端不再长期保持按钮 loading，避免误判为“卡在 pending”。
  - 修复高并发选股场景下偶发 `libmini_racer` 进程崩溃：`DataFetcherManager` 对 `AkshareFetcher` 的日线抓取增加线程安全串行保护，避免 V8 地址池重复初始化触发 `FATAL:address_pool_manager.cc(67)`。
  - 港股/美股扫描范围新增自动扩展：在保留原配置项的基础上，自动补充行业板块直达范围，并在范围不足 20 时补充分层代表池，保证 `scopes` 接口每个市场可返回 20+ 扫描范围。
  - 港股/美股板块补充东方财富“知名港股/知名美股分类池”作为 live enrich 候选来源；当全市场列表接口不稳定时，仍可用该来源参与板块匹配补全。
  ...
  - 公式引擎新增同花顺/通达信运行时函数兼容：支持扩展码表 `DYNAINFO(3/4/5/6/7/8/10/11/12/35/39/40/41)`、`FINANCE(1/2/6/7/30/33/34/35/37/38/40/41/42/46/47)`、`NAMELIKE()`；并支持多行 `XG:` 标签表达式；`DRAWICON/DRAWTEXT/.../COLOR*` 等绘图语句在选股执行时自动忽略，不再导致校验失败。
